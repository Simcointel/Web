...
export function RiskView({ phase, retail }: SuiteViewProps) {
  return (
    <div className="card p-10 !shadow-none border-surface-200 dark:border-surface-800">
       ...
       <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-6">
          {RESOURCES.filter(r => r.retailInfo && r.retailInfo.length > 0).slice(0, 24).map(res => {
              const sat = retail?.retail ? (Object.entries(retail.retail).find(([k]) => k.toLowerCase() === res.name.toLowerCase())?.[1]?.saturation ?? 1.0) : 1.0;
             return (
