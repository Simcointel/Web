...
export function MacroPage() {
  ...
  const { data: latest, loading: lLoading, error: lError, refresh: lRefresh } = useApiPoll(() => api.macro.latest(realm), 30000, [realm]);
  const { data: history, loading: hLoading } = useApiPoll(() => api.macro.history(realm, { limit: 120 }), 60000, [realm]);
  const { data: indexes, loading: iLoading } = useApiPoll(() => api.macro.indexes(realm, 60), 60000, [realm]);
  const { data: inflation, loading: infLoading } = useApiPoll(() => api.macro.inflation(realm, 60), 60000, [realm]);
  const { data: phases } = useApiPoll(() => api.macro.phases(realm), 60000, [realm]);
